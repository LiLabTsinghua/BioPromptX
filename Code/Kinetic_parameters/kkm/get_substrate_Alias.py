...
def locate_full_substrate_name_prompt(text, substrate_name):
    ...
    Please check if this substrate corresponds to any other name in the text, and if there are other corresponding names, please return all other names.

    The output must be a JSON object in the following format:
        {{
            "other names": "(name1,name2,name3)"
        }}
    ...
    If other name of the input substrate is not mentioned in the article, outputs the input name directly.
        Example：
        input: "NAD+"
        {{
            "other names": "(NAD+)"
    ...
    Please ensure:
    All other names are taken from the article and represent the same substrate as the substrate name I provided!
    All other names of the subs
    """
            ...
            for param in combined_params:
                ...
                if substrate_name:
                    ...
                    for substrate in substrates:
                        ...
                        Alia_names.append(
                            result.get("other names", substrate))  # Default to the original name if not found

                    # Combine full names into a single string to match the original 'substrate&co'
                    combined_Alia_name = "; ".join(Alia_names)

                    # Append the data
                    extracted_data.append({
                        "value": value,
                        "substrate&co": substrate_name,  # Keep the original substrate&co intact
                        "substrate full name": param.get('substrate full name', None),
                        "Alias name": combined_Alia_name,
...
if __name__ == '__main__':
    process_json(
        'text_10paper_kinetic_combined_substrate_fullname_enzyme.json',
        'text_10paper_kinetic_combined_substrate_fullname_alias_enzyme.json'
    )
