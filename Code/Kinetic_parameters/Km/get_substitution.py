...
def locate_enzyme_ec_number(text, parameter_value, strain, enzyme):
    ...
    prompt = f"""
        Text: ''' {text} '''
        Based on the above text, the following enzyme kinetic parameter Km value, strain name and key gene of corresponding enzyme has been extracted.

        Km value: {parameter_value}
        strain name: {strain}
        Key gene of the enzyme: "{enzyme}"

        Your task is to find the the Km value's corresponding substitution(amino acid modification, mutation, replacement) from the article I provided.
        ...
        When the text does not exactly mention the Km value's corresponding substitution.
        output: 
        {{
