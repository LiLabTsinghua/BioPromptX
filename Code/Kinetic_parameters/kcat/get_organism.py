...
def locate_full_substrate_name_prompt(text, parameter_value, enzyme):
    ...
    prompt = f"""
        ...
        Your task is to find the organism corresponding to the enzyme and kcat value I provided in the article.
    ...
    with tqdm(total=len(data), desc="Processing", unit="paper") as pbar:
        for index, content in enumerate(data, start=start_index):
            ...
            for param in combined_params:
                ...
                if enzyme:
                    # Extract the full name of the substrate (Note: Function name suggests substrate, but logic extracts organism)
