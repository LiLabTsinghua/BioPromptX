...
def locate_full_substrate_name_prompt(text, parameter_value, enzyme):
    ...
    prompt = f"""
        Text: ''' {text} '''
        Based on the above text, the following enzyme kinetic parameter kcat value and key gene of corresponding enzyme has been extracted.

        kcat value: {parameter_value}
        Key gene of the enzyme: "{enzyme}"

        Your task is to find the strain name corresponding to the enzyme and kcat value I provided in the article.
    ...
    with tqdm(total=len(data), desc="Processing", unit="paper") as pbar:
        for index, content in enumerate(data, start=start_index):
            ...
            for param in combined_params:
                ...
                if enzyme:
                    # Extract the full name of the substrate (Note: Function name suggests strain extraction based on prompt logic)
...
if __name__ == '__main__':
    process_json(
        'SC_kinetic_combined_substrate_fullname_enzyme_organism.json',
