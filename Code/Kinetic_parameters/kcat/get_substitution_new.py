...
def locate_enzyme_ec_number(text, parameter_value, strain, enzyme):
    """
    Extract the substitution information for the specified enzyme from the text using the API.
    """
    ...
    prompt = f"""
        ...
        Your task is to find the substitution(mutation, replacement) corresponding to the enzyme, strain name and kcat value I provided in the article.
        ...
        When the text does not mention the corresponding substitution information.
        output: 
        {{
            "substitution": "wild type"
        }}
        
        Attention, Typically the format of the substitution information is as follows: letter (single-letter code for the original amino acid) number (position of the amino acid in the protein sequence) letter (single-letter code for the replaced amino acid).
        Please ensure that the format of the substitution information is wild type or the format mentioned above.
    """
...
def process_json(input_path='nature.json', output_path='output_results.json', temp_path='temp_results.json'):
    ...
    # Check if a temporary file exists to resume processing from the last saved point
...
if __name__ == '__main__':
    process_json(
        ...
        'SC_kinetic_combined_substrate_fullname_enzyme_organism_strain_EC_substitution.json')
